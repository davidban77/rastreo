#!/bin/sh
# install.sh — Install rastreo pre-built binaries.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/davidban77/rastreo/main/install.sh | sh
#
# Environment variables:
#   RASTREO_VERSION      — version to install (e.g. "v0.3.0"). Defaults to latest release.
#   RASTREO_INSTALL_DIR  — installation directory. Defaults to /usr/local/bin.
#
# Supported platforms:
...
main() {
    ...
    say "installing rastreo ${version} for ${target}"

    # Resolve install directory
    install_dir="${RASTREO_INSTALL_DIR:-/usr/local/bin}"

    # Build download URLs
    tarball_name="rastreo-${version}-${target}.tar.gz"
    ...
    # Install binaries
    install_binary "${tmp_dir}/rastreo" "${install_dir}/rastreo"
    install_binary "${tmp_dir}/rastreo-server" "${install_dir}/rastreo-server"

    say ""
    say "rastreo ${version} installed successfully"
    say "  rastreo:        ${install_dir}/rastreo"
    say "  rastreo-server: ${install_dir}/rastreo-server"
    say ""
    say "run 'rastreo --help' to get started"
}

say() {
    printf 'rastreo-install: %s\n' "$1"
}
...
get_latest_version() {
    ...
    # Matches "tag_name": "v0.3.0" and extracts v0.3.0
...
ensure_dir() {
    ...
    if command -v sudo > /dev/null 2>&1; then
        ...
        err "cannot create directory ${dir} — run as root or set RASTREO_INSTALL_DIR"
    fi
}
...
install_binary() {
    ...
    if cp "$src" "$dest" 2>/dev/null; then
        ...
        err "cannot install to ${dest} — run as root or set RASTREO_INSTALL_DIR"
    fi
}
