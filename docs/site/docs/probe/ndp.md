...
The feature pulls in `pnet_datalink` and `pnet_packet` from the libpnet family, plus `ipnetwork` for subnet arithmetic. The dependency graph is identical to the `arp` feature; enabling both incurs the same transitive compile cost as enabling either alone.
