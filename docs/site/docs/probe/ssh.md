...
The [rules classifier](../discover/classification.md) runs by default, so an `SshBanner` matching a baked-in rule (Ubuntu, Debian, FreeBSD) populates `DeviceRecord.ssh_version` (SSH software identifier — for example `OpenSSH_9.3p1`). `platform` and `os_version` stay `null`: the banner names the sshd binary, and a Nokia SR Linux switch answers from a Debian-packaged OpenSSH. Opt into [banner heuristics](../discover/classification.md#banner-heuristics-opt-in) on a server estate where the banner really does identify the machine.
