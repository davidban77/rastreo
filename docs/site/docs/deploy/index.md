...
Topics covered here include CLI invocation patterns, the `rastreo-server` REST API (`POST /scans`, `GET /healthz`, `GET /readyz`), Docker Compose usage (the development stack ships in the repo), and Helm chart deployment with the optional ServiceMonitor for Prometheus scraping.
