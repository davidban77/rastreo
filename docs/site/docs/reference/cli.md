...
The same rule reaches the other two subcommands, and the rule is this: **`-q` never removes the only channel carrying the answer, and never removes a failure. Everything else goes.** That includes things nothing else tells you — the counters above are one, and so is the `hint:` line, which is where the remedy lives: an error message names what is wrong, and the hint names the flag or the rebuild that fixes it. So a `-q` run that fails gives you the error without the remedy. When the error alone does not tell you what to do, run the same command again without the flag. [`validate`](../discover/validate.md#reading-the-output) drops the per-scenario `ok` lines and the `all valid` summary — both on stdout — and keeps the reason each invalid scenario gives on stderr, because that reason and the exit code are the answer; a clean file under `-q` writes nothing to either stream and exits `0`. [`catalog list`](../discover/catalog.md#listing-catalog-scenarios) drops the `no catalog scenarios found` note on stderr and keeps the listing on stdout, because nothing else carries the names. Records and the `--dry-run` plan are the same case as the listing: no verbosity flag has ever reached either.
...
The completion glyph is blue on a clean run and turns yellow when the run covered less ground than you asked for — it was cancelled, records were quarantined to a dead-letter destination, a target contributed no addresses, or a scenario in a multi-scenario file failed or was skipped. Probe faults alone do not tint it; they are expected on any real scan and are counted in the `faults:` field.
...
A target that contributed no addresses is not an error while any other target was probed: the run exits `0`, the completion banner counts it as `unresolvable: N`, and `-v` names which targets. Like every other fact the completion banner carries — probe faults, quarantined records, a cancelled run — `-q` silences it, so pair `-q` with `--dry-run --format json` if you need the answer without the banner: each target carries its own `"resolution"`, and `unresolvable` is one of the three states. It becomes exit `1` only when **every** target contributed no addresses, because then the scan probed nothing at all. See [Names with no addresses](../discover/targets.md#names-with-no-addresses) and [Names rastreo cannot look up](../discover/targets.md#names-rastreo-cannot-look-up).
