...
| Variant | Display message | Common cause | Likely fix |
|---|---|---|---|
| `ResolverError::DnsLookupFailed { name }` | `DNS lookup failed for <name>` (`source`: the lookup failure itself — `request timed out`, `error response: SERVFAIL`, `no connections available`, and so on) | The system resolver could not be reached, timed out, or returned a server error. The failure is about the nameserver rather than the target, so it aborts the scan instead of skipping the name. | Check `/etc/resolv.conf`; try `dig <name>` from the same host. In containers, verify the container's resolver can reach your internal DNS — see [Troubleshooting](../integrate/troubleshooting.md#dns-resolution-failures). |
