...
| Variant | Display message | Common cause | Likely fix |
|---|---|---|---|
| `ResolverError::DnsLookupFailed { name }` | `DNS lookup failed for <name>` (`source`: the lookup failure itself — `no records found`, `request timed out`, `error response: SERVFAIL`, `no connections available`, and so on) | A lookup that produced no addresses arrives as this variant whatever the reason, so read the `source`. A `no records found` source means the network answered that the name has none. The scan skips that target and runs on. Any other source says nothing about the target, so the scan aborts: a timeout, `SERVFAIL`, a resolver rastreo could not reach. | Check `/etc/resolv.conf`; try `dig <name>` from the same host. In containers, verify the container's resolver can reach your internal DNS — see [Troubleshooting](../integrate/troubleshooting.md#dns-resolution-failures). |
| `ResolverError::DnsNoRecords { name }` | `DNS lookup returned no records for <name>` | The name exists but has no `A` or `AAAA` records. This is not fatal to a `rastreo discover` scan: the target is skipped and reported as unresolvable — see [Names with no addresses](../discover/targets.md#names-with-no-addresses). It surfaces as an error only to a library caller supplying its own `Resolver`. | Confirm the name has the expected records (`dig <name> A`, `dig <name> AAAA`). |
| `ResolverError::UnqueryableTargetName { name }` | `target name <name> is neither a DNS name nor an IP address, so it was never looked up` | The target is not a name any DNS query can carry. Common cases: a `host:port` pair, a bracketed address, a zone-suffixed IPv6 address such as `fe80::1%eth0`, and a name with an empty or over-long part. Not fatal: the target is skipped, reported as unresolvable, and the rest of the scan runs. The message is the `reason` field on the skip warning. | Fix the target. See [Names rastreo cannot look up](../discover/targets.md#names-rastreo-cannot-look-up) for what a name may contain. Underscored hostnames such as `core_sw01.lab` are not in this group and need no change. |
...
| Variant | Display message | Common cause | Likely fix |
|---|---|---|---|
| `ResumeError::IdentityFuserNotResumable` | `scenario is not resume-safe: the identity fuser correlates records across the whole scan, which cannot be reconstructed from a partial checkpoint prefix` | `--checkpoint` on a scenario using the [`identity`](../discover/identity.md) fuser. | Switch the scenario to the `direct` or `mib_enrichment` fuser, or run it without a checkpoint. |
...
| `ResumeError::NoCheckpointToResume { path }` | `no checkpoint to resume at <path>; resuming continues a checkpoint an earlier run wrote to this path` | `--resume` pointed at a path with no checkpoint — often a typo, or a previous run that completed and removed it. | Check the path. A completed scan deletes its checkpoint; run without `--resume` to start fresh. The CLI prints that as a `hint:` line above the error. |
