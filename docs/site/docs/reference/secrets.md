...
An unresolvable reference does not crash the pod. Sink construction fails, `/readyz` returns `503` with `reason: "sink_unreachable"`, and `last_probe_error` names the variable and the config path — `environment variable NATS_PASS referenced in sink config is not set`. The server rebuilds the sink from the file on every probe interval, so a `!file` secret that is mounted late resolves on its own; an environment variable set after the process started does not, because its value is fixed for the process lifetime.
