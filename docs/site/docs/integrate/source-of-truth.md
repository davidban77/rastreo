...
Fingerprint signals identify the software running on a device and what role that device plays. `SshBanner`, `SnmpSysDescr`, `SnmpSysName`, `SnmpSysObjectId`, and `HttpBanner` share one rule vocabulary and can feed either phase of the [rules classifier](../discover/classification.md) — as a `platform_rules` pattern, or as a `signal_match` role rule. `SnmpSysObjectId` additionally feeds the role phase through `sys_object_id_prefix` subtree matching, and `OpenPort` feeds it through `ports_open`. The rules classifier runs by default, so `DeviceRecord.platform`, `DeviceRecord.os_version`, `DeviceRecord.ssh_version`, `DeviceRecord.http_server`, `DeviceRecord.http_version`, and `DeviceRecord.role` are populated directly and the raw signals remain on the record as history.

Expect these fields to carry values as soon as a probe collects a signal a baked-in rule matches — an SNMP `sysDescr` sets `platform` and `os_version`, an SSH banner sets `ssh_version`, and `[22, 179]` open together sets `role: router`. The shipped tables classify only on evidence about the thing the field names, and leave the field at `null` on a guess, because `null` is the one value that is safe downstream: most reconcilers, including the three sketched below, overwrite a field whose value differs rather than filling only empty ones. A single open port is a guess about `role`; an SSH or HTTP banner is a guess about `platform`, because it names the software on that port rather than the device's OS. A populated `platform` or `role` will replace whatever an operator set by hand, so decide deliberately whether your reconciler should write those two fields at all — and consider gating the write on the object being unset, or on `confidence` clearing a threshold.
...
|---|---|---|---|
| `SshBanner` | Feeds `ssh_version` (SSH software identifier, e.g. `OpenSSH_9.3p1`) when the SSH banner rules match. Leaves `platform` and `os_version` alone: the banner names the sshd binary, not the device's OS. Example value: `"SSH-2.0-OpenSSH_9.3p1 Ubuntu-1ubuntu3"`. | Store `ssh_version` in a custom field (`rastreo_ssh_version`) and keep the raw banner for audit. | Store `ssh_version` on a dedicated string attribute; keep the raw banner in a `ssh_banner` string attribute. |
| `SnmpSysDescr` | Feeds `platform` and `os_version` when the SNMP `sysDescr` rules match. Covers Cisco IOS / IOS-XR / NX-OS, Juniper JUNOS, Arista EOS, Nokia SR Linux, Linux. The only signal a baked-in rule claims a platform from. | `dcim.Device.platform` foreign key + a `dcim.Device.software_version` custom field (or the built-in `Platform.version` if you model it that way). | `platform` attribute + a version string attribute on the device kind. |
| `SnmpSysName` | User-defined rules only; no defaults ship. Set `platform` / `os_version` from your naming convention if you have one, or drive `role` from it with a `signal_match` role rule (`-spine\d+$` → `spine`). | Same shape as `SnmpSysDescr` above for platform; a role derived from the hostname lands on `dcim.Device.role` (or `device_role`). | Same shape as `SnmpSysDescr` above for platform; role on the standard role attribute. |
| `HttpBanner` | Feeds `http_server` (web-server product, e.g. `nginx` or `Apache`) and `http_version` (product version, e.g. `1.24.0`) when the `Server:` header matches the HTTP rules. Leaves `platform` and `os_version` alone: nginx runs on Linux, BSD, and Windows. Covers `nginx/*` and `Apache/*`. | Store `http_server` and `http_version` in custom fields (`rastreo_http_server`, `rastreo_http_version`). Useful mainly for hosts and appliances that speak HTTP as the front door. | Dedicated `http_server` and `http_version` string attributes. |
| `SnmpSysObjectId` | Feeds `role` when a user-supplied `sys_object_id_prefix` role rule matches the OID subtree, and `platform` / `os_version` when a user-supplied `platform_rules` entry matches the OID text — the enterprise arc (`6527` in `1.3.6.1.4.1.6527.1.20.26`) names the vendor unambiguously. No baked-in rules of either kind ship for this signal: no public vendor MIB tree cleanly maps sub-prefixes to device roles, so users supply rules against their own fleet's OIDs. See `classification.md#baked-in-role-rules` for the rationale. Example value: `"1.3.6.1.4.1.9.1.2050"`. | `dcim.Device.role` (or `device_role`) foreign key from `DeviceRecord.role`, and `dcim.Device.platform` from `DeviceRecord.platform`. Keep the raw OID in a custom field like `rastreo_sys_object_id` for audit. | Standard role and platform attributes + a `sys_object_id` string attribute. |
