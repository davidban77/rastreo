...
| `SnmpSysObjectId` | Feeds `role` when a `sys_object_id_prefix` role rule matches. Baked-in prefixes cover Cisco IOS / IOS-XR / Catalyst, Juniper MX / EX, and Arista EOS. Example value: `"1.3.6.1.4.1.9.1.2050"`. | `dcim.Device.role` (or `device_role`) foreign key from `DeviceRecord.role`. Keep the raw OID in a custom field like `rastreo_sys_object_id` for audit. | Standard role attribute + a `sys_object_id` string attribute. |
