...
RUN RUST_TARGET=$(cat /tmp/rust-target) && \
    ...
    cargo build --release --target "${RUST_TARGET}" --features kafka,http -p rastreo -p rastreo-server 2>/dev/null || true
...
RUN RUST_TARGET=$(cat /tmp/rust-target) && \
    if [ -s /tmp/cross-env ]; then export $(cat /tmp/cross-env); fi && \
    cargo build --release --target "${RUST_TARGET}" --features kafka,http -p rastreo -p rastreo-server
