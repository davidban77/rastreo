...
`expand_fuser_selection` builds the one nesting `FuserConfig::validate` accepts by sorting the selection on `nesting_rank` (base innermost, enrichers over it, the identity correlator outermost) and folding `compose` over it, so a caller cannot express an invalid tree. A selection naming no base nests over `default_fuser_config`. Adding a `FuserConfig` variant means adding a `FuserKind` and writing its `nesting_rank` and `compose` arms — both matches are exhaustive over the enum, so a new kind cannot be silently expanded away for want of a place to stack. The compiler forces the rest of the vocabulary too: `all()` returns `&'static [FuserKind; FUSER_KIND_COUNT]`, so a variant added without bumping the count is a type error, and `label`, `is_compiled_in`, and `required_feature` are exhaustive matches. `from_label` is the one obligation the compiler does not force — it matches `&str` with a `_ => None` fallback, so a new kind silently fails to parse and only `every_label_round_trips_through_from_label` (`fuser/selection.rs`) catches it.

On `FuserKind`, `is_compiled_in` and `required_feature` are inherent methods on the kind (`kind.is_compiled_in()`), not free functions, so a caller reading two taxonomies in one file has no name collision to alias around. `ProbeKind` still exposes both as free functions in `prober/selection.rs`; converting them is pending.
