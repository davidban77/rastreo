...
`Scheduler::run_scan -> Vec<TargetScan>` is the target-outer contract: it runs every prober against every target, bounds in-flight probes to the concurrency cap, paces probe starts, and returns each fully-scanned target's outcomes — each tagged with its prober pass index — in input-target order. It still buffers the whole `Vec<TargetScan>` before returning; streaming those scans out as each target completes is the next step. The pipeline consumes this buffer and reconstructs the batch's order-sensitive results: records in input-target order, and `first_probe_error` at the smallest `(prober_pass_index, target_index)` key (the fault a prober-outer scan would have latched first). Order-insensitive metrics (`probe_attempts`, `error_counts`, per-kind tallies, DLQ) are sums and do not depend on scan order. A cancelled scan stops priming new probes, lets in-flight probes drain, and returns only fully-scanned targets, so the pipeline emits complete records for the targets that finished.
