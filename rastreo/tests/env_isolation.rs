...
fn a_backtrace_exported_into_the_suite_never_reaches_the_binary() {
    ...
    assert!(!output.status.success(), "expected the refusal to exit 1");
...
fn every_test_spawns_the_binary_through_the_shared_helper() {
    ...
    assert!(
        offenders.is_empty(),
        "spawn through common::rastreo(), which clears the environment; a hand-built command \
         inherits whatever the shell or the CI job exported: {offenders:?}"
    );
}
