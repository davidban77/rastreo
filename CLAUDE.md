...
- **Test hangs**: a test may fail, but it may never hang. Sequence a race on the event itself, never on a sleep long enough to "surely" have won it, and bound every wait — including the ones that are not `await`s. A blocking thread and the blocking pool the runtime joins when it drops are the two that no `tokio::time::timeout` reaches. A hang burns the CI job's whole timeout budget and reports as infrastructure trouble, so nobody reads it as a test failure.
- **Ambient environment**: a test never reads or mutates the process environment to reach its assertion. A binary crate parses argv in-process through its own `argv::parse_without_env`, which strips clap's `env =` sources; `xtask/tests/argv_parsing.rs` walks every crate's sources and fails on any clap entry point that takes an argv outside the files defining that parse. A new binary crate needs its own copy and an entry in that guard's exemption list.
